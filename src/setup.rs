...
/// 2. `~/.ghidrai/ghidra/support/analyzeHeadless` (managed install location)
...
/// Download and install Ghidra to `~/.ghidrai/ghidra`.
pub async fn setup_ghidra() -> Result<()> {
    ...
    println!("[adapter:ghidra] installed to ~/.ghidrai/ghidra");
