                        ...
                        },
                        {
                            ...
                            "inputSchema": {
                                ...
                                "properties": {
                                    ...
                                    "command": { "type": "string", "enum": ["ping", "list_functions", "decompile", "get_function_signature", "get_xrefs", "search_strings"] },
                                    "args": { "type": "object", "description": "JSON arguments for the command (e.g. {\"function\":\"main\"} or {\"query\":\"password\"})" }
                                },
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let status_block = Paragraph::new(status_text)
                .block(Block::default().title(" ⚙️ SYSTEM STATUS ").borders(Borders::ALL).border_style(Style::default().fg(Color::Magenta)).style(Style::default().fg(Color::White)));
            ...
            let logs_block = List::new(log_items)
                .block(Block::default().title(" 📡 MCP EVENT LOGS (Live) ").borders(Borders::ALL).border_style(Style::default().fg(Color::Cyan)).style(Style::default().fg(Color::Green)));
            f.render_widget(logs_block, top_chunks[1]);

            // Bottom: Ghidra Tasks
            let header = Row::new(vec!["Task ID", "Ghidra Action", "Status", "Live Progress"])
                .style(Style::default().bg(Color::Blue).fg(Color::White).add_modifier(ratatui::style::Modifier::BOLD)).height(1).bottom_margin(1);
            ...
            for task in st.tasks.iter().rev() {
                ...
                rows.push(Row::new(vec![
                    ...
                    Cell::from(task.status.clone()).style(Style::default().fg(color).add_modifier(ratatui::style::Modifier::BOLD)),
                    Cell::from(task.progress.clone()).style(Style::default().fg(Color::LightCyan)),
                ]));
            }
            if rows.is_empty() { rows.push(Row::new(vec!["-", "No Active Ghidra Tasks", "-", "-"]).style(Style::default().fg(Color::DarkGray))); }
            ...
            let table = Table::new(rows, [Constraint::Percentage(10), Constraint::Percentage(25), Constraint::Percentage(15), Constraint::Percentage(50)])
                .header(header)
                .block(Block::default().title(" 🔍 GHIDRA HEADLESS TASKS ").borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow)));
